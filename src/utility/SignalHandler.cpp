...
// Taken and adapted from https://stackoverflow.com/a/7582555
...
#ifdef _WIN32
	bool const success = SetConsoleCtrlHandler(WIN32_handleFunc, TRUE);
	if (!success) {
		LOGGER()->error("Failed to enable ConsoleCtrl handler!");
	}
	assert(success);
	(void)success;
#endif //_WIN32
...
#ifdef _WIN32
	bool const success = SetConsoleCtrlHandler(WIN32_handleFunc, FALSE);
	if (!success) {
		LOGGER()->error("Failed to disable ConsoleCtrl handler!");
	}
	assert(success);
	(void)success;
#else
